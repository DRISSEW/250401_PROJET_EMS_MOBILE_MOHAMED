...
const DropDownMenu = () => {
  ...
  const updateValuesOnly = useCallback(async () => {
    try {
      const response = await fetch(`${API_URL}?apikey=${apiKey}`);
      if (!response.ok) throw new Error('Failed to fetch live values');
      const updatedFeeds: FeedItem[] = await response.json();
  ...
  if (!apiKey) {
    return (
      <View style={[styles.centered, { backgroundColor: colors.background }]}>
        <Text style={[styles.errorText, { color: colors.error }]}>No API key provided</Text>
      </View>
    );
  }
